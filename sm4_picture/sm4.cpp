 ...
 * return the contents of encryption/decryption contents.
 */
long sm4F_without_rk(unsigned long x0, unsigned long x1, unsigned long x2, unsigned long x3)
{
	return (x0 ^ sm4Lt(x1 ^ x2 ^ x3));
}
/// <summary>
...
void sm4_round_without_rk(unsigned char input[16], unsigned char output[16])
{
	...
	GET_ULONG_BE(ulbuf[2], input, 8);
	GET_ULONG_BE(ulbuf[3], input, 12);
	ulbuf[4] = sm4F_without_rk(ulbuf[0],ulbuf[1],ulbuf[2],ulbuf[3]);
	PUT_ULONG_BE(ulbuf[1], output, 0);
	PUT_ULONG_BE(ulbuf[2], output, 4);
	PUT_ULONG_BE(ulbuf[3], output, 8);
	PUT_ULONG_BE(ulbuf[4], output, 12);
}
/// <summary>
///  sgim4 中使用的R函数	
/// </summary>
...
void sgim4_round(unsigned char* input, unsigned char* key, unsigned char* output)
{
	...
	sm4_round_without_rk(key, result);
